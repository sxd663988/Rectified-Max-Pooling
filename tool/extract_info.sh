#!/bin/bash
# this script is used to batch extraction.
# you can modify the local str to other keys.
...
function ergodic(){
  ...
  do
    local path=$1"/"$file
    local str="AverageImageCalFactor= "  #TargetAz= 
    # echo ${1##*/}
    # echo ${file%%.*}
    if test -f $path; then
      # echo $path
      ...
      grep -a -m1 $str $path | sed "s/$str//g" > "./"${2}"/"${1##*/}"/"${file%%.*}".txt"  
    fi
  done
}
